...
import com.egame.gameServer.common.db.SubordinateDBManager;
import com.egame.gameServer.common.entity.PropertyKey;
import com.egame.gameServer.common.extension.GameExtension;
import com.egame.gameServer.common.util.GameUtil;

import mytest.DataRow;
...
public class GameQueryUtil {
	...
	public static String getLoactionById(int id) {
		String gameCode = GameUtil.getStringByKey("game.gameCode");
		Object[] objects = { gameCode, id };
		String sql = "select loaction from line where gameCode =? and id =? ";
		DataRow dr = SubordinateDBManager.getInstance().querySole(sql, objects);
	...
	public static JSONObject getAllData() {
		...
		ArrayList<DataRow> dr = SubordinateDBManager.getInstance().query(sql, objects);
	...
	public static int getIdByLoaction(String location) {
		...
		if (PropertyKey.MYERA.equals(gameCode) || PropertyKey.SPRINGDAY.equals(gameCode)) {
	...
	public static List<Integer> getIdsByLoaction(List<StringBuffer> locations) {
		...
		String gameCode = GameExtension.instance.gameCode;
//		String gameCode = GameUtil.getStringByKey("game.gameCode");
		...
		ArrayList<DataRow> dr = SubordinateDBManager.getInstance().query(sql.toString(), objects);
